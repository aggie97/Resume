    ...
    <div>
      ...
      <ul className="about">
        <li id="9c296626-9dce-433e-a46b-d0ee31347a63">
          <h4>능동적으로 문제를 찾아 해결하는 것을 좋아합니다.</h4>
          요구 사항을 충족시키는 것에 그치지 않고 항상 왜? 라는 단어를 붙이며
          능동적인 자세로 제품 개발에 임하려고 합니다. 디자인이나 기획의
          잠재적인 장애를 조기 발견하여 QA 과정에서 발생하는 (스프린트 단위 당)
          이슈 티켓의 갯수를 약 20% 줄였던 경험이 있습니다.
        </li>
        <li>
          <h4>문서화하고 공유하는 것을 좋아합니다.</h4>
          제품 개발 투입 시기가 앞당겨져 함께 입사한 팀원들이 환경 적응에
          어려움을 겪었습니다. 이를 위해 복잡한 비지니스 로직 흐름을 파악,
          정리하여 공유 문서를 작성하였고 이를 토대로 4주간 사내 스터디를
          주도했습니다.
        </li>
        <li>
          <h4>반복되는 로직이나 프로세스를 최소화하는 일을 좋아합니다.</h4>
          중복되는 코드를 공통 컴포넌트와 커스텀 훅으로 대체하여 코드 라인을
          기존 대비 50%으로 줄였습니다. 모노 레포로 작업 시에 백엔드, 패키지
          변경 사항이 생길 때 마다 개발 환경 동기화를 위해 커맨드를 입력하여
          적용시켜야 했습니다. 이러한 과정에서 발생하던 커맨드 8 줄을 1 줄로
          줄였던 경험이 있습니다.
        </li>
        <li>
          <h4>저는 이렇게 일해왔습니다.</h4>
          개발팀에서 FE 개발자로 일하며 프로젝트의 2차 기획까지 거치게
          되었습니다. 애자일한 환경 속에서 지속적인 기획과 디자인의 변동, QA
          피드백에 대한 대응을 하며 여러가지의 성공과 실패를 경험했습니다. 6개월
          동안 개발팀에서 일하며 느낀 것은{" "}
          <b>유지 보수를 위해 코드를 작성해야 한다는 것</b>과{" "}
          <b>
            원활한 소통을 통해 완성도를 높여 프로젝트의 성공을 이끌어내는 것
          </b>
          이 중요하다는 것입니다.
        </li>
      </ul>
      ...
      <div style={{ marginBottom: "1.5rem" }}>
        <div className="exp_header">
          ...
          <span>2023.09.04 ~ 2024.03.01</span>
        </div>
        <div>
          <ul>
            <li>로그인, 로그아웃, 회원가입 구현</li>
            <li>페이지 라우팅 권한 분기 구현</li>
            <li>데이터 시각화(대시보드, 차트, 테이블) 개발 담당</li>
            <li>
              앱 자체 파일 브라우저 구현(Chonky.js + AWS S3 JavaScript SDK)
            </li>
          </ul>
        </div>
      </div>
